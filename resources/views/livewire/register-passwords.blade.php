<div>
    ...
    <div class="flex mt-1 mb-2">
        ...
        <div class="flex items-center place-content-end ml-1">
            <x-button wire:click="generatePassword" type="button">{{ __('Generar') }}</x-button>
        </div>
    </div>

    <span class="text-sm">
        <span class="font-semibold">{{ __('Seguridad de la contraseña') }}:</span>
        {{ $strengthLevels[$strengthScore] ?? 'Débil' }}
    </span>

    <progress value="{{ $strengthScore }}" max="4" class="w-full bg-blue-600 h-2.5 rounded-full"></progress>
    <div class="mt-4">
        <x-label for="password_confirmation" value="{{ __('Confirma Contraseña') }}" />
        <x-input id="password_confirmation" wire:model="passwordConfirmation" class="block mt-1 w-full" type="password"
