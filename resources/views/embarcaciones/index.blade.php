...
<x-app-layout>
    ...
    <div class="row">
        ...
        @php
            ...
            $embarcaciones_representadas = auth()->user()->embarcaciones_representadas();
        @endphp
        {{-- embarcaciones del propietario nacionales --}}
        @if ($embarcaciones->count() > 0)
            <div class="row">
                <h2 class="h3 mb-2 mt-3 text-black text-uppercase">Embarcaciones principales
                </h2>
                        ...
                        <div
                            ...
                            <div class="card-body">
                                ...
                                <div class="grid grid-cols-1 gap-3">
                                    ...
                                    <div class="border-bottom text-dark border-secondary mt-2 text-center">
                                        <p style=""><strong>{{ __('MATRÍCULA') }}:</strong> {{ $emb->matricula }}
                                        </p>
                                    </div>
                                    <div class="border-bottom text-dark border-secondary mt-2 text-center">
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body text-center">
                                    ...
                                    <form id="conduce-form-{{ $emb->id }}"
                                        action="{{ route('conduces.createpost') }}" method="POST" class="d-none">
                                        @csrf
                                        <input type="hidden" name="emb" value="{{ $emb->matricula }}">
                                    </form>
                                    ...
                                    <form id="salidas-form-{{ $emb->id }}"
                                        action="{{ route('salidas.createpost') }}" method="POST" class="d-none">
                                        @csrf
                                        <input type="hidden" name="emb" value="{{ $emb->matricula }}">
                                    </form>
                        ...
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                <div class="modal-header bg-blue-900">
                                    ...
                                    <button type="button" class="btn-close text-white font-bold"
        ...
        @if (
            $embarcaciones_representadas->count() > 0 &&
                auth()->user()->representante &&
                auth()->user()->representante->conciente == 1)
            <div class="row">
                <h2 class="h3 mb-2 mt-3 text-black text-uppercase">Embarcaciones representadas</h2>
                @foreach ($embarcaciones_representadas as $emb_rep)
                    @php
                        $inteligencia = EmbarcacioneController::inteligencias($emb_rep->matricula);
                    @endphp
                    <div class="col-xxl-3 col-lg-6">
                        {{-- listado de las embarcaciones disponibles por usuario --}}
                        <div
                            class="card widget-flat border @if (strtotime($emb_rep->fecha_validez->format('d-m-Y')) >= strtotime(\Carbon\Carbon::now()->format('d-m-Y'))) border-custom @else border-custom-red @endif border-5 rounded sombra">
                            <div class="card-body">
                                <span class="badge text-gray-800">{{ __('Nacional') }}</span>
                                <div class="float-end">
                                    <i
                                ...
                                <h5 class="fw-normal mt-0" title="Revenue">
                                    <strong>{{ __('Fecha de expiración') }}:</strong>
                                    @if (strtotime($emb_rep->fecha_validez->format('d-m-Y')) >= strtotime(\Carbon\Carbon::now()->format('d-m-Y')))
                                        <small
                                            class="badge bg-success me-1 h2 py-1">{{ $emb_rep->fecha_validez->format('d-m-Y') }}</small>
                                    @else
                                        <small
                                            class="badge bg-danger me-1 h2 py-1">{{ $emb_rep->fecha_validez->format('d-m-Y') }}</small>
                                    @endif
                                </h5>
                                <div class="grid grid-cols-1 gap-3">

                                    <div class="border-bottom text-dark border-secondary mt-2 text-center">
                                        <p style=""><strong>{{ __('NOMBRE') }}:</strong>
                                            {{ $emb_rep->nombre }}
                                        </p>
                                    </div>
                                    <div class="border-bottom text-dark border-secondary mt-2 text-center">
                                        <p style=""><strong>{{ __('MATRÍCULA') }}:</strong>
                                            {{ $emb_rep->matricula }}
                                        </p>
                                    </div>
                                    <div class="border-bottom text-dark border-secondary mt-2 text-center">
                                        <p style=""><strong>{{ __('PUERTO REGISTRO') }}:</strong>
                                            {{ strtoupper($emb_rep->estacionamiento) }}
                                        </p>
                                    </div>
                            ...
                            <div class="card-footer">
                                <small>{{ __('Ultima solicitud') }}:
                                    {{ !empty($emb_rep->movimiento->last())
                                        ? $emb_rep->movimiento->last()->created_at->format('d-m-Y') .
                                            ' (' .
                                            $emb_rep->movimiento->last()->created_at->diffForHumans([
                                                'parts' => 2,
                                                'join' => ' y ',
                                            ]) .
                                            ')'
                                ...
                                <h3 class="mt-0 mb-1 ">
                                    <strong>{{ __('Solicitudes realizadas') }}:</strong>
                                    <small
                                        class="badge bg-blue-900 me-1 py-1">{{ $emb_rep->movimiento->count() }}</small>
                                </h3>
                                <h3>
                                    Estado de la solicitud:
                                    @if (!$emb_rep->movimiento->isEmpty())
                                        @if ($emb_rep->movimiento->last()->estado == 'Enviado')
                                            <span
                                                class="bg-yellow-100 text-black text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-yellow-700 dark:text-yellow-300">{{ __('Enviado') }}</span>
                                        @elseif ($emb_rep->movimiento->last()->estado == 'En proceso')
                                            <span class="badge bg-blue-700 me-1 py-1">{{ __('En proceso') }}</span>
                                        @elseif ($emb_rep->movimiento->last()->estado == 'Aprobado')
                                            <span class="badge bg-green-700 me-1 py-1">{{ __('Aprobado') }}</span>
                                        @elseif ($emb_rep->movimiento->last()->estado == 'Rechazado')
                                            <span class="badge bg-red-800 me-1 py-1">{{ __('Rechazado') }}</span>
                                        @elseif ($emb_rep->movimiento->last()->estado == 'Cancelado')
                                            <span class="badge bg-red-800 me-1 py-1">{{ __('Cancelado') }}</span>
                                        @endif
                                    @else
                                        <span class="badge bg-secondary me-1 py-1">{{ __('Sin solicitudes') }}</span>
                                ...
                                <div class="d-grid mt-2 col-lg-12">
                                    @if (empty($inteligencia))
                                        @if ($emb_rep->manual != 1)
                                            @if (strtotime($emb_rep->fecha_validez->format('d-m-Y')) >= strtotime(\Carbon\Carbon::now()->format('d-m-Y')))
                                                @if ($emb_rep->estado_movimiento == 0 || $emb_rep->estado_movimiento == 3)
                                                    <button type="button"
                                                        class="items-center px-3 py-2 bg-azulito border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 ml-1 block"
                                                        data-bs-toggle="modal"
                                                        data-bs-target="#option-mov-modal-{{ $emb_rep->id }}">{{ __('SOLICITAR') }}
                                                    </button>
                                                @elseif ($emb_rep->estado_movimiento == 1)
                                                    <!-- Botón con spinner -->
                                                    <button type="button"
                                                        class="request_open items-center px-3 py-2 bg-green-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:bg-green-700 active:bg-green-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition ease-in-out duration-150 ml-1 block">
                                                        <span v-if="cargando">
                                                ...
                                                @elseif ($emb_rep->estado_movimiento == 2)
                                                    <button type="button"
                                                        class="moving items-center px-3 py-2 bg-yellow-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-700 focus:bg-yellow-700 active:bg-yellow-900 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition ease-in-out duration-150 ml-1 block">
                                                        <span v-if="cargando">
                                                            <div
                    ...
                    <!-- Standard modal -->
                    <div id="option-mov-modal-{{ $emb_rep->id }}" class="modal fade" tabindex="-1" role="dialog"
                        aria-labelledby="standard-modalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header mx-auto">
                                ...
                                <div class="modal-body text-center">
                                    <a href="{{ route('despachos.createpost') }}"
                                        class="items-center px-3 py-2 bg-azulito border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-600 focus:bg-blue-700 active:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 ml-1 url_despacho"
                                        onclick="event.preventDefault(); document.getElementById('despacho-form-{{ $emb_rep->id }}').submit();">{{ __('DESPACHO') }}

                                    </a>
                                    <form id="despacho-form-{{ $emb_rep->id }}"
                                        action="{{ route('despachos.createpost') }}" method="POST" class="d-none">
                                        @csrf
                                        <input type="hidden" name="emb" value="{{ $emb_rep->matricula }}">
                                    </form>

                                    <a href="{{ route('conduces.createpost') }}"
                                        class="items-center px-3 py-2 bg-yellow-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-600 focus:bg-yellow-700 active:bg-yellow-900 focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:ring-offset-2 transition ease-in-out duration-150 ml-1 url_conduce"
                                        onclick="event.preventDefault(); document.getElementById('conduce-form-{{ $emb_rep->id }}').submit();">{{ __('CONDUCE') }}</a>

                                    <form id="conduce-form-{{ $emb_rep->id }}"
                                        action="{{ route('conduces.createpost') }}" method="POST" class="d-none">
                                        @csrf
                                        <input type="hidden" name="emb" value="{{ $emb_rep->matricula }}">
                                    </form>
                    ...
                    <!-- Standard modal fotos-->
                    <div id="option-pic-modal-{{ $emb_rep->id }}" class="modal fade" tabindex="-1" role="dialog"
                        aria-labelledby="standard-modalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                <div class="modal-header bg-blue-900">
        ...
        @endif
        {{-- embarcaciones temporales internacionales --}}
        @if ($emb_internacionales->count() > 0)
            <div class="row">
                <h2 class="h3 mb-2 mt-3 text-black text-uppercase">Embarcaciones internacionales</h2>
                @foreach ($emb_internacionales as $embi)
                    <div class="col-xxl-3 col-lg-6">
                        ...
                        <div
                            ...
                            <div class="card-body">
                                ...
                                <div class="border-bottom text-dark border-secondary mt-2 text-center">
                                    <p style=""><strong>{{ __('NOMBRE') }}:</strong> {{ $embi->nombre }}
                                    </p>
                                </div>
                                <div class="border-bottom text-dark border-secondary mt-2 text-center">
                                    <p style=""><strong>{{ __('MATRÍCULA') }}:</strong>
                    ...
                    <!-- Standard modal -->
                    <div id="option-mov-modal-{{ $embi->id }}-int" class="modal fade" tabindex="-1"
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <form id="conduce-form-{{ $embi->id }}" action="{{ route('conduces.createpost') }}"
                                    ...
                                    <a href="{{ route('salidas.createpost') }}"
                                        class="items-center px-3 py-2 bg-green-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:bg-green-700 active:bg-green-900 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 transition ease-in-out duration-150 ml-1 url_conduce"
                                        onclick="event.preventDefault(); document.getElementById('salidasint-form-{{ $embi->id }}').submit();">{{ __('SALIDAS') }}</a>

                                    <form id="salidasint-form-{{ $embi->id }}"
                                        action="{{ route('salidas.createpost') }}" method="POST" class="d-none">
                                        @csrf
                                        <input type="hidden" name="emb" value="{{ $embi->matricula }}">
                                    </form>
