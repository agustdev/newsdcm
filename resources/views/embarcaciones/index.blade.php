<x-app-layout>
    ...
    <div class="row">
            ...
            <div
                ...
                <div class="card-body">
                    ...
                    <h5 class="text-muted fw-normal mt-0" title="Revenue"><strong>Fecha de expiración:</strong>
                        ...
                        <small class="badge bg-success me-1 h2 py-1">{{ $emb->fecha_validez->format('d-m-Y')
                            }}</small>
                        @else
                        <small class="badge bg-danger me-1 h2 py-1">{{ $emb->fecha_validez->format('d-m-Y')
                            }}</small>
                        @endif
                    </h5>
                    <div class="col-lg-12">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body text-center">
                        ...
                        <a href="{{ route('salidas.createpost') }}"
                            ...
                            onclick="event.preventDefault(); document.getElementById('salidas-form-{{ $emb->id }}').submit();">SALIDAS</a>

                        <form id="salidas-form-{{ $emb->id }}" action="{{ route('salidas.createpost') }}" method="POST"
                            class="d-none">
                            @csrf
                            <input type="hidden" name="emb" value="{{ $emb->matricula }}">
                        </form>
        ...
        <div class="col-xxl-3 col-lg-6">
            ...
            <div
                ...
                <div class="card-body">
                    ...
                    <h5 class="text-muted fw-normal mt-0" title="Revenue"><strong>Fecha de expiración:</strong>
                        @if(strtotime($embi->fecha_validez->format('d-m-Y')) >=
                        strtotime(\Carbon\Carbon::now()->format('d-m-Y')))
                        <small class="badge bg-success me-1 h2 py-1">{{ $embi->fecha_validez->format('d-m-Y')
                            }}</small>
                        @else
                        <small class="badge bg-danger me-1 h2 py-1">{{ $embi->fecha_validez->format('d-m-Y')
                            }}</small>
                        @endif
                    </h5>
                    <div class="col-lg-12">
                        <p class="mb-2 text-muted ">
                            <span class="badge badge-outline-success me-1 font-weight-bold py-1">
                                <i class="uil-ship uil-16-plus me-1" style="font-size: 24px;"></i> <span
                                    style="vertical-align: super; font-size: 14px;">{{ $embi->nombre }}
                                </span>
                            </span>
                        </p>
                        <p class="mb-2 text-muted ">
                            <span class="badge badge-outline-info me-1 py-1">
                                <i class="mdi mdi-card-account-details mdi-48px me-1"></i> <span
                                    style="vertical-align: super; font-size: 14px;">{{ $embi->matricula }}</span>
                            </span>
                        </p>
                        <p class="text-muted ">
                            <span class="badge badge-outline-danger me-1 py-1">
                                <i class="mdi mdi-card-text mdi-36px me-1"></i>
                                <span style="vertical-align: super; font-size: 14px;">CHASIS: {{ $embi->no_chasis
                                    }}</span>
                            </span>
                        </p>
                    </div>
                ...
                <div class="card-footer">
                    <small>Ultima solicitud: {{ !empty($embi->movimiento->last()) ?
                        $embi->movimiento->last()->created_at->diffForHumans() : '' }}</small>
                    ...
                    <div class="d-grid mt-2 col-lg-12">
                        ...
                        <button type="button"
                            class="items-center px-3 py-2 bg-blue-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 ml-1 block"
                            data-bs-toggle="modal" data-bs-target="#option-mov-modal-{{ $embi->id }}">SOLICITAR</button>
                        @else
                        <button type="button" disabled
                            class="items-center px-3 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 ml-1 disabled:opacity-25 block">SOLICITAR</button>
                        @endif
        ...
        <!-- Standard modal -->
        <div id="option-mov-modal-{{ $embi->id }}" class="modal fade" tabindex="-1" role="dialog"
            aria-labelledby="standard-modalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body text-center">
                        <a href="{{ route('despachos.createpost') }}"
                            class="items-center px-3 py-2 bg-blue-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 ml-1 url_despacho"
                            onclick="event.preventDefault(); document.getElementById('despacho-form-{{ $embi->id }}').submit();">DESPACHO

                        </a>
                        <form id="despacho-form-{{ $embi->id }}" action="{{ route('despachos.createpost') }}"
                            method="POST" class="d-none">
                            @csrf
                        ...
                        <a href="{{ route('conduces.createpost') }}"
                            class="items-center px-3 py-2 bg-yellow-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-700 focus:bg-yellow-700 active:bg-yellow-900 focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:ring-offset-2 transition ease-in-out duration-150 ml-1 url_conduce"
                            onclick="event.preventDefault(); document.getElementById('conduce-form-{{ $embi->id }}').submit();">CONDUCE</a>
                        ...
                        <a href="{{ route('salidas.createpost') }}"
                            class="items-center px-3 py-2 bg-green-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:bg-green-700 active:bg-green-900 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 transition ease-in-out duration-150 ml-1 url_conduce"
                            onclick="event.preventDefault(); document.getElementById('salidas-form-{{ $embi->id }}').submit();">SALIDAS</a>

                        <form id="salidas-form-{{ $embi->id }}" action="{{ route('salidas.createpost') }}" method="POST"
                            class="d-none">
                            @csrf
                            <input type="hidden" name="emb" value="{{ $embi->matricula }}">
                        </form>
