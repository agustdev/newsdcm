<x-app-layout>
    @section('titulo', __('Listado de usuarios operativos'))
    @push('css')
            ...
            <div class="card shadow-xl">
                <div class="card-header">
                    <h2 class="h3 text-slate-600">
                        ...
                        <a href="{{ route('usuarios.navieras.create') }}"
                            class="inline-flex items-center justify-center px-3 py-2 bg-azulito border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-600 active:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 transition ease-in-out duration-150 float-end"><i
                                class="mdi mdi-plus mdi-18px"></i>{{ __('Nuevo usuario') }}</a>
                    </h2>
                </div>
                <div class="card-body">
                    @if ($usuarios->count() > 0)
                        <table id="table-users" class="table dt-responsive table-striped nowrap w-100">
                            <thead>
                            ...
                            <tbody>
                                @foreach ($usuarios as $usuario)
                                    <tr>
                                        ...
                                        <td>
                                            <button
                                                class="inline-flex items-center justify-center px-3 py-2 bg-yellow-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-500 active:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition ease-in-out duration-150">Editar</button>
                                            <form action="{{ route('usuarios.navieras.destroy', $usuario) }}"
                                                class="inline-block delete_users" method="POST">
                                                @method('DELETE')
                                                @csrf
                                                <button
                    ...
                    @else
                        <div class="alert alert-warning">
                            No tiene ningun usuario operativo
                        </div>
                    @endif
                </div>
            </div>
    ...
    @push('js')
        @if (Session::has('info'))
            <script script script>
                Swal.fire(
                    'Buen trabajo!', 'Se ha eliminado el usuario con exito', 'success'
                )
            </script>
        @endif
        <!-- Datatables js -->
