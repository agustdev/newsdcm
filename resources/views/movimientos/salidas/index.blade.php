            ...
            <div class="card">
                <div class="card-header">
                    <h2 class="h3 text-slate-600">
                        {{ __('Listado de Solicitudes de Salida') }}
                        <a href="{{ route('movimientos.salidas.create') }}"
                    ...
                    <table id="table-despacho" class="table dt-responsive table-striped nowrap w-100">
                        ...
                        <tbody>
                            @foreach ($salidas as $sal)
                                <tr>
                                    <td>{{ $sal->id }}</td>
                                    <td>{{ $sal->fecha->format('d-m-Y') }}</td>
                                    <td>{{ $sal->matricula }}</td>
                                    <td>
                                        @if ($sal->estado == 'Aprobado')
                                            <span
                                                class="bg-green-100 text-green-600 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-green-700 dark:text-green-300">{{ $sal->estado }}</span>
                                        @elseif ($sal->estado == 'Rechazado' or $sal->estado == 'Cancelado')
                                            <span
                                                class="bg-red-100 text-red-600 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-red-700 dark:text-red-300">{{ $sal->estado }}</span>
                                        @elseif ($sal->estado == 'Enviado')
                                            <span
                                                class="bg-yellow-100 text-yellow-600 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-yellow-700 dark:text-yellow-300">{{ $sal->estado }}</span>
                                        @elseif ($sal->estado == 'En proceso')
                                            <span
                                                class="bg-blue-100 text-blue-600 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-700 dark:text-blue-300">{{ $sal->estado }}</span>
                                        @endif
                                    </td>
                                    <td>
                                        <a href="{{ route('movimientos.salidas.show', $sal) }}"
                                            class="inline-flex items-center justify-center px-3 py-2 bg-azulito border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-600 active:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                            <i class="uil-eye"></i>
                                        </a>
                                    </td>
                                    <td>
                                        {{ $sal->created_at->format('d-m-Y h:i:s') }}
                                    </td>
                                    <td>
                                        <div class="tooltip-container">
                                            ...
                                            @endphp
                                            @if (!in_array($sal->estado, $estados))
                                            ...
                                            @if (!in_array($sal->estado, $estados))
                                                <form id="despacho-cancel"
                                                    action="{{ route('movimientos.salidas.destroy', $sal) }}"
