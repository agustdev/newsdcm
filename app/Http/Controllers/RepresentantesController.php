<?php

namespace App\Http\Controllers;

use App\Models\Representantes;
use Illuminate\Http\Request;

class RepresentantesController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        return view('representantes.index');
    }

    public function consentimiento(Request $request)
    {
        Representantes::where('documento', '=', auth()->user()->documento)
            ->orWhere('documento', '=', auth()->user()->documento_formateado)->update([
                'conciente' => $request->conciente
            ]);
        // dd($request->all());
        return redirect()->route('embarcaciones.index')->with('success', 'Consentimiento actualizado correctamente.');
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(Request $request)
    {
        //
    }

    /**
     * Display the specified resource.
     */
    public function show(Representantes $representantes)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(Representantes $representantes)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, Representantes $representantes)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(Representantes $representantes)
    {
        //
    }
}
